...
function SettingsContent() {
  ...
  function handleSignatureChange(e) {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 200 * 1024) { alert("Signature must be under 200KB."); return; }
    const reader = new FileReader();
  ...
  function removeStaff(role) {
    if (!confirm("Remove this staff? Mobile and PIN will both be cleared.")) return;
    if (role === "receptionist") { setPhoneR(""); setPinR(""); }
    if (role === "pharmacy") { setPhoneP(""); setPinP(""); }
    if (role === "psychologist") { setPhonePsy(""); setPinPsy(""); }
  }

  async function handleSave() {
    if (phoneR && phoneR.length !== 10) { alert("Receptionist mobile must be 10 digits"); return; }
    if (phoneP && phoneP.length !== 10) { alert("Pharmacy mobile must be 10 digits"); return; }
    if (phoneR && pinR.length !== 6) { alert("Receptionist PIN must be 6 digits"); return; }
    if (phoneP && pinP.length !== 6) { alert("Pharmacy PIN must be 6 digits"); return; }
    if (hasPsychologist) {
      if (phonePsy && phonePsy.length !== 10) { alert("Psychologist mobile must be 10 digits"); return; }
      if (phonePsy && pinPsy.length !== 6) { alert("Psychologist PIN must be 6 digits"); return; }
    }
  ...
  const inputCls = "w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 mt-1";
  ...
  return (
    <div className="pt-4 pb-8">
      ...
      <div className="bg-white rounded-2xl shadow p-4 mb-4">
        ...
        {logo ? (
          <div className="flex flex-col items-center gap-3">
            <img src={logo} alt="Clinic logo" className="h-20 object-contain rounded-lg border border-gray-200" />
            <button type="button" onClick={() => setLogo("")} className="text-xs text-red-400">
              Remove logo
            </button>
          </div>
        ) : (
      ...
      <div className="bg-white rounded-2xl shadow p-4 mb-4">
        ...
        {signature ? (
          <div className="flex flex-col items-center gap-3">
            <img src={signature} alt="Signature" className="h-20 object-contain rounded-lg border border-gray-200" />
            <button type="button" onClick={() => setSignature("")} className="text-xs text-red-400">
              Remove signature
            </button>
          </div>
        ) : (
          <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl py-6 cursor-pointer hover:border-violet-400 transition">
            <span className="text-2xl mb-1">✍️</span>
            <span className="text-sm text-gray-500">Tap to upload signature</span>
            <span className="text-xs text-gray-400 mt-1">Max 200KB · PNG/JPG</span>
      ...
      <div className="bg-white rounded-2xl shadow p-4 mb-4 flex flex-col gap-3">
        ...
        {[
          { label: "Clinic Name", value: clinicName, set: setClinicName, placeholder: "" },
          { label: "Doctor Name", value: doctorName, set: setDoctorName, placeholder: "Dr. Firstname Lastname" },
          { label: "Qualification", value: qualification, set: setQualification, placeholder: "MBBS, MD (Psychiatry)" },
          { label: "Registration Number", value: regNo, set: setRegNo, placeholder: "Medical Council Reg. No." },
          { label: "Clinic Phone", value: clinicPhone, set: setClinicPhone, placeholder: "Clinic contact number", type: "tel" },
      ...
      <div className="bg-white rounded-2xl shadow p-4 mb-4 flex flex-col gap-4">
        ...
        {/* Receptionist */}
        {[
          { role: "receptionist", label: "Receptionist", phone: phoneR, setPhone: setPhoneR, pin: pinR, setPin: setPinR, showPin: showPinR, setShowPin: setShowPinR },
          { role: "pharmacy", label: "Pharmacy", phone: phoneP, setPhone: setPhoneP, pin: pinP, setPin: setPinP, showPin: showPinP, setShowPin: setShowPinP },
        ].map(({ role, label, phone, setPhone, pin, setPin, showPin, setShowPin }) => (
          <div key={role} className="border border-gray-100 rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-gray-700">{label}</p>
              {phone && (
                <button type="button" onClick={() => removeStaff(role)}
            ...
            <input
              ...
              onChange={(e) => handlePhoneChange(e.target.value, setPhone, pin, setPin)}
              className={`${inputCls} mb-2`}
            />
            <label className="text-xs text-gray-500">PIN (6 digits)</label>
            <div className="flex gap-2 mt-1">
              <input
                type={showPin ? "text" : "password"} value={pin} maxLength={6}
                inputMode="numeric"
        ...
        <div className="border-t border-gray-100 pt-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">Enable if clinic has a psychologist</p>
            </div>
            <button type="button" onClick={() => setHasPsychologist((p) => !p)}
              ...
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${hasPsychologist ? "translate-x-6" : "translate-x-1"}`} />
            </button>
          </div>
          ...
          {hasPsychologist && (
            <div className="border border-gray-100 rounded-xl p-3 mt-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-gray-700">Psychologist Login</p>
                {phonePsy && (
                  <button type="button" onClick={() => removeStaff("psychologist")}
                    className="text-xs font-semibold text-white bg-red-500 px-3 py-1 rounded-lg">
                    Remove
                  </button>
                )}
              ...
              <label className="text-xs text-gray-500">Mobile Number</label>
              <input
                type="tel" inputMode="numeric" value={phonePsy} maxLength={10}
                autoComplete="off" placeholder="10-digit mobile"
                onChange={(e) => handlePhoneChange(e.target.value, setPhonePsy, pinPsy, setPinPsy)}
                className={`${inputCls} mb-2`}
              />
              <label className="text-xs text-gray-500">PIN (6 digits)</label>
              <div className="flex gap-2 mt-1">
                <input
                  type={showPinPsy ? "text" : "password"} value={pinPsy}
                  maxLength={6} inputMode="numeric"
                  onChange={(e) => setPinPsy(e.target.value.replace(/\D/g, "").slice(0, 6))}
                  className="flex-1 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400"
                />
                <button type="button" onClick={() => setShowPinPsy((p) => !p)}
                  className="text-lg px-3 border border-gray-200 rounded-xl text-gray-500">
                  {showPinPsy ? "🙈" : "👁️"}
                </button>
              </div>
