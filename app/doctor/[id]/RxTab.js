...
function doseOptionsFor(medName, MEDICINES_BY_TIER) {
  ...
  for (const cond of CONDITIONS) {
    ...
    if (hit) return (hit.dose || "").split(",").map((s) => s.trim()).filter(Boolean);
  }
  return [];
}
...
export default function RxTab({
  ...
  saving, saved,
  onSave, onSendToPsy, saveAsTemplate,
  templates, brandsMap, frequentMeds,
  addedNames, pickedSalts, pickedCount,
  showPicker, setShowPicker,
  search, setSearch,
  selectedCondition, setSelectedCondition,
  tierTab, setTierTab,
  pickerList, interactions,
  togglePick, addAllPicked, addCustomMed,
  newSalt, setNewSalt,
  newBrand, setNewBrand,
  newSaltCat, setNewSaltCat,
  MEDICINES_BY_TIER,
}) {
  function addMedicine() {
    setMedicines((prev) => [...prev, { name: "", dose: "", timing: [], duration: "7 days", food: "After food", brand: "" }]);
  }
  ...
  function removeMedicine(index) {
    setMedicines((prev) => {
      ...
      return next.length === 0 ? [{ name: "", dose: "", timing: [], duration: "7 days", food: "After food", brand: "" }] : next;
    });
  }
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        ...
        <input
          type="text" list="diagnosis-list" value={diagnosis}
        ...
        <datalist id="diagnosis-list">
          {["Insomnia","Migraine","Epilepsy","Depression","Anxiety","Bipolar Disorder","Schizophrenia","OCD","ADHD","PTSD","Substance Use Disorder","Sexual Health"].map(v => <option key={v} value={v} />)}
        </datalist>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow p-4">
          ...
          <div className="flex flex-col gap-2">
            {Object.entries(templates).map(([diag, meds]) => (
              <div key={diag} className="flex justify-between items-center px-3 py-2 rounded-xl border border-gray-200 bg-gray-50">
                <span className="text-sm font-semibold text-gray-700 flex-1">{diag}</span>
                <span className="text-xs text-gray-400 mr-3">{meds.length} medicines</span>
                <button type="button" onClick={() => { setDiagnosis(diag); setMedicines(meds.map((m) => ({ ...m }))); }}
                  className="text-xs text-indigo-600 border border-indigo-300 rounded-lg px-2 py-1 hover:bg-indigo-50 mr-1">Load</button>
                <button type="button" onClick={async () => {
                  if (!confirm(`Delete template "${diag}"?`)) return;
                  const updated = { ...templates }; delete updated[diag];
                  await fetch("/api/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ templates: JSON.stringify(updated) }) });
                }} className="text-xs text-red-500 border border-red-300 rounded-lg px-2 py-1 hover:bg-red-50">Delete</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {Object.keys(frequentMeds).length > 0 && (
        <div className="bg-white rounded-2xl shadow p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(frequentMeds).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([name]) => {
              const already = addedNames.has(name.toLowerCase());
              const mapped = brandsMap[name] || {};
              return (
                <button key={name} type="button" disabled={already}
                  onClick={() => {
                    if (already) return;
        ...
        {showPicker && (
          <>
            ...
            <div className="flex gap-1 mb-2">
              {[["all","All","bg-gray-800"],["latest","🆕 Latest","bg-amber-500"],["common","🔹 Common","bg-indigo-600"]].map(([key,label,bg]) => (
                <button key={key} type="button" onClick={() => setTierTab(key)}
            ...
            {(() => {
              const grouped = {};
              for (const med of pickerList) { const cls = med.class || "Other"; if (!grouped[cls]) grouped[cls] = []; grouped[cls].push(med); }
              return (
                <div className="flex flex-col gap-3 max-h-[400px] overflow-y-auto">
                  {Object.keys(grouped).length === 0 && <p className="text-xs text-gray-400 text-center py-4">No match.</p>}
                    ...
                    <div key={cls}>
                      ...
                      <div className="grid grid-cols-1 gap-1.5">
                        {grouped[cls].map((med, idx) => {
                          ...
                          return (
                            <button key={`${cls}-${med.name}-${idx}`} type="button" onClick={() => togglePick(med)} disabled={already}
                              ...
                              <div className="flex items-center gap-2 min-w-0">
                                <span className={`w-4 h-4 flex items-center justify-center rounded border text-[10px] shrink-0 ${already ? "bg-emerald-500 border-emerald-500 text-white" : picked ? "bg-white border-white text-indigo-600" : "border-gray-300"}`}>
                                  {(picked || already) && "✓"}
                                </span>
            ...
            {pickedCount > 0 && (
              <div className="sticky bottom-0 mt-3 -mx-4 -mb-4 px-4 py-3 bg-white border-t border-gray-200 flex gap-2">
                <button type="button" onClick={() => {}} className="px-3 py-2 text-xs border border-gray-300 rounded-lg text-gray-600">Clear</button>
                <button type="button" onClick={addAllPicked} className="flex-1 bg-indigo-600 text-white py-2 rounded-lg font-semibold text-sm">Add {pickedCount} Selected →</button>
              </div>
            )}
            <div className="mt-3 pt-3 border-t border-gray-200">
              <p className="text-[11px] font-semibold text-gray-600 mb-1">Medicine not in list? Add it here:</p>
              <div className="flex gap-2">
                <input type="text" value={newSalt} onChange={(e) => setNewSalt(e.target.value)} placeholder="Salt name" className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400" />
                <input type="text" value={newBrand} onChange={(e) => setNewBrand(e.target.value)} placeholder="Brand" className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400" />
                <select value={newSaltCat} onChange={(e) => setNewSaltCat(e.target.value)} className="border border-gray-300 rounded-lg px-2 py-2 text-xs focus:outline-none max-w-[120px]">
                  {CONDITIONS.map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <button type="button" onClick={addCustomMed} className="w-full mt-2 bg-emerald-600 text-white py-2 rounded-lg font-semibold text-sm">+ Add to library</button>
            </div>
          </>
        )}
      </div>
      ...
      {interactions.length > 0 && (
        <div className="flex flex-col gap-2">
          {interactions.map((w, i) => (
            <div key={i} className={`rounded-2xl p-4 border-2 ${w.severity === "danger" ? "bg-red-50 border-red-300" : "bg-amber-50 border-amber-300"}`}>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-lg">{w.severity === "danger" ? "⛔" : "⚠️"}</span>
                <span className={`text-xs font-bold uppercase ${w.severity === "danger" ? "text-red-700" : "text-amber-700"}`}>{w.severity === "danger" ? "Dangerous Interaction" : "Caution"}</span>
              </div>
              <p className="text-sm text-gray-700"><strong>{w.drugs.join(" + ")}</strong></p>
              <p className="text-xs text-gray-600 mt-1">{w.message}</p>
            </div>
          ))}
          <p className="text-[10px] text-gray-400 px-1">Automated safety check — final decision is the doctor&apos;s responsibility.</p>
        </div>
      )}
            ...
            return (
              <div key={i} className="border border-gray-200 rounded-xl p-3 flex flex-col gap-2">
                ...
                <input type="text" value={med.name} onChange={(e) => updateMedicine(i, "name", e.target.value)}
                  placeholder="Salt name" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-400" />
                <div className="flex gap-2">
                  {doseOpts.length > 0 && med.dose !== "__custom__" ? (
                    <select value={doseOpts.includes(med.dose) ? med.dose : ""} onChange={(e) => { updateMedicine(i, "dose", e.target.value); setTimeout(() => applyBrandIfKnown(i), 0); }}
                    ...
                    <input type="text" value={med.dose === "__custom__" ? "" : med.dose} onChange={(e) => updateMedicine(i, "dose", e.target.value)}
                      placeholder="Dose (e.g. 10mg)" className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-400" />
                  )}
                </div>
                <div>
                  <label className="text-[10px] text-gray-400">Brand {known && <span className="text-emerald-500">• saved</span>}</label>
                  <input type="text" value={med.brand || ""} onChange={(e) => updateMedicine(i, "brand", e.target.value)}
                    placeholder="Enter your brand (e.g. Nexito)" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400 mt-0.5" />
                </div>
                <div className="flex flex-wrap gap-1">
                  {MEDICINE_TIMINGS.map((t) => (
                    <button key={t} onClick={() => toggleTiming(i, t)}
                      className={`text-xs px-2 py-1 rounded-full border transition ${med.timing.includes(t) ? "bg-emerald-500 text-white border-emerald-500" : "border-gray-300 text-gray-600"}`}>{t}</button>
                  ))}
                </div>
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {[7, 14, 30, 60].map((d) => (
            <button key={d} type="button" onClick={() => quickFollowup(d)} className="text-xs border border-gray-300 rounded-full px-3 py-1 text-gray-600">+{d}d</button>
          ))}
        ...
        {followupDate && <p className="text-xs text-emerald-700 mt-2">Reminder will be sent on {followupDate}</p>}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => onSave(false)} disabled={saving} className="bg-emerald-600 text-white py-3 rounded-xl font-semibold text-sm hover:bg-emerald-700 disabled:opacity-60 transition">
          {saving ? "Saving..." : "Save"}
        </button>
        <button onClick={() => onSave(true)} disabled={saving} className="bg-gray-800 text-white py-3 rounded-xl font-semibold text-sm hover:bg-gray-900 disabled:opacity-60 transition">
          Save & Print
        </button>
      </div>

      <button type="button" onClick={onSendToPsy} disabled={saving}
