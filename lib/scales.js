...
export const PHQ9 = {
  ...
  scoreFn(answers) {
    ...
    return {
      ...
      label: b.label,
      color: b.color,
      alert: q9 > 0 ? "Q9 positive — assess suicide risk" : "",
    };
  },
};
...
export const DASS21 = {
  ...
  scoreFn(answers) {
    ...
    const sum = (idx) =>
      idx.reduce((s, i) => s + (Number(answers[i]) || 0), 0) * 2;
    const dep = sum(dIdx),
      anx = sum(aIdx),
      str = sum(sIdx);
...
export const MDQ = {
  ...
  followups: [
    {
      key: "cooccur",
      text: "If you ticked more than one above, did several happen during the same period?",
      options: YES_NO,
    },
    {
      key: "severity",
      ...
      detail: [
        {
          name: "Symptoms",
          score: yesCount,
          label: `${yesCount}/13 yes`,
          color: yesCount >= 7 ? "amber" : "emerald",
        },
      ],
    };
  },
...
// Clinician-administered. Items 1,2,3,4,5,6,10,11,12,13,14,15,16,17 = 0-4 range (some only 0-2)
// Standard scoring: 0-7 Normal, 8-13 Mild, 14-18 Moderate, 19-22 Severe, ≥23 Very Severe
export const HAMD17 = {
  id: "hamd17",
  name: "HAMD-17",
  title: "Hamilton Depression (HAMD-17)",
  intro:
    "Clinician-rated. Rate each item based on patient interview and observation over the past week.",
  options: [], // custom per-question — handled in scoreFn
  max: 52,
  questions: [
    "1. Depressed Mood — Sadness, hopelessness, helplessness, worthlessness",
    "2. Guilt — Self-reproach, ideas of guilt, past behaviour",
    "3. Suicide — Feels life not worth living, suicidal ideas or attempts",
    "4. Insomnia Early — Difficulty falling asleep",
    "5. Insomnia Middle — Restless, disturbed during the night",
    "6. Insomnia Late — Early waking, unable to fall asleep again",
    "7. Work & Activities — Loss of interest, decreased productivity",
    "8. Psychomotor Retardation — Slowness of thought, speech, impaired concentration",
    "9. Agitation — Restlessness, picking at hands, hair",
    "10. Anxiety (Psychic) — Tension, irritability, fearful",
    "11. Anxiety (Somatic) — Physiological concomitants of anxiety",
    "12. Somatic GI — Loss of appetite, heavy feeling in abdomen",
    "13. Somatic General — Limb heaviness, fatigue, loss of energy",
    "14. Genital Symptoms — Loss of libido, menstrual disturbances",
    "15. Hypochondriasis — Self-absorption, preoccupation with bodily health",
    "16. Weight Loss — Rated by history",
    "17. Insight — Acknowledges being depressed/ill",
  ],
  // Item max values (0-based index): items 0,1,2,6,7,8,9,10 = max 4; rest = max 2
  itemMax: [4, 4, 4, 2, 2, 2, 4, 4, 4, 4, 4, 2, 2, 2, 4, 2, 2],
  customOptions: [
    // Items 0,1,2,6,7,8,9,10,14 — 0 to 4
    [
      { label: "0 — Absent", value: 0 },
      { label: "1 — Doubtful / trivial", value: 1 },
      { label: "2 — Mild", value: 2 },
      { label: "3 — Moderate", value: 3 },
      { label: "4 — Severe", value: 4 },
    ],
    // Items 3,4,5,11,12,13,15,16 — 0 to 2
    [
      { label: "0 — Absent", value: 0 },
  ...
  // Which index uses which option set: true = 0-4, false = 0-2
  use5: [
    true,
    true,
    true,
  ...
  scoreFn(answers) {
    const total = answers.reduce((s, v) => s + (Number(v) || 0), 0);
    const b = band(total, [
      { min: 0, max: 7, label: "Normal / Remission", color: "emerald" },
      { min: 8, max: 13, label: "Mild Depression", color: "lime" },
      { min: 14, max: 18, label: "Moderate Depression", color: "amber" },
      { min: 19, max: 22, label: "Severe Depression", color: "orange" },
      { min: 23, max: 52, label: "Very Severe", color: "red" },
    ]);
    ...
    return {
      ...
      alert:
        q3 >= 3
          ? "Item 3 (Suicide) critical — immediate assessment required"
          : "",
    };
  },
};

export const SCALES = [PHQ9, GAD7, PHQ2, DASS21, MDQ, HAMD17];
